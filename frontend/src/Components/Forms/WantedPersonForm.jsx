...
const WantedPersonForm = ({wanted, refresh, crud}) => {
    ...
    useEffect(()=>{
        if (wanted) {
            setName(wanted.name)
            setGender(wanted.gender)
            setDob(wanted.dob)
            setDatetime(wanted.datetime)
            setAddress(wanted.address)
            setMother(wanted.mother)
            setIncident(wanted.incident)
            setNationality(wanted.nationality)
            setPassportid(wanted.passportid)
            setPhoto(wanted.photo)
            setTel(wanted.telephone)
        }
    },[wanted])
    ...
    const handleWantedPersonInfo = async (e) => {
        e.preventDefault()

        try {
            const response = await fetch(`/api/wanted/new`, {
                method: 'POST',
            ...
            if (response.ok) {
                setError(null);
                setSuccess("Wanted person added to database!");
                console.log("Response returned! ", json);
                refresh()
            } else { console.log("Response error!") }
        } catch (e) {
            console.log("Error submitting Wanted Person info")
            console.log(e)
        }
    }
    ...
    return (
        <div>
            <form className="wantedperson-form" onSubmit={handleWantedPersonInfo}>
                <h3 className="highlight">{crud}</h3>

                <div className="image-preview">
                    <label htmlFor="">Photo</label>
                    <img className="citizen-photo" src={photo} alt="wanted-person" />
